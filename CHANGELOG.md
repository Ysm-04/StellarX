...
- **Dialog 在窗口尺寸变化时的行为进一步明确：**
  - 对话框在窗口 resize、最大化、还原等场景下只会重新计算位置并保持居中显示，不再跟随窗口拉伸自身尺寸。
  - 对话框的尺寸仍然只由标题、消息内容和按钮区域决定，使行为更稳定、更可预期。

- **Dialog 标题绘制方式调整：**
  - 对话框标题改为由 `Dialog` 自身直接绘制，不再依赖单独的 `Label` 子控件。
  - 这减少了一层额外的背景快照与重绘时序，提升了标题区域在复杂场景下的稳定性。

- **Window 尺寸接口语义调整：**
  - `Window::getWidth()` / `Window::getHeight()` 现在返回当前已实际生效的客户区尺寸。
  - 若需要获取 resize 收口前记录的待处理尺寸，请使用：
    - `Window::getPendingWidth()`
    - `Window::getPendingHeight()`

- **对话框遮挡下的交互语义收紧：**
  - 非模态对话框覆盖区域内的底层控件不再继续响应鼠标事件。
  - 这使被对话框遮挡的按钮、输入框等控件不再出现误 hover、误点击或绘制层级异常。

### ✅修复

- **修复直接最大化窗口时可能出现的黑背景与控件残影：**
  - 修复了某些初始窗口尺寸下，直接点击最大化后新暴露区域显示为黑色、已有控件出现残影的问题。
  - 现在大跨度但合法的窗口尺寸变化将正常完成 resize 收口与整场景重绘，不再被错误跳过。

- **修复 Dialog 在 resize、重定位与关闭过程中的背景残留：**
  - 修复模态与非模态对话框在窗口尺寸变化后，标题、关闭按钮、功能按钮周围偶发出现残影的问题。
  - 修复对话框关闭后旧背景恢复不完整、周围出现脏边或残留内容的问题。

- **修复对话框遮挡场景下的底层控件误交互：**
  - 修复非模态对话框打开时，底层被完全覆盖或部分覆盖的按钮仍可能接收鼠标事件的问题。
  - 修复底层按钮在被对话框遮挡后，hover 状态不能及时解除的问题。
  - 修复对话框关闭后底层控件 hover 状态恢复不及时的问题。

- **修复 Dialog 标题区域的背景快照异常：**
  - 修复标题区域在个别重绘时序下抓取到底层背景的异常现象，避免标题区域出现错层或背景穿透。

- **修复基础控件与容器的一批边界问题：**
  - 修复 `Canvas / TabControl` 在事件已经被消费后仍继续向下分发的问题。
  - 修复 `Table` 在空数据、空表头、列数不一致等输入下的边界风险。
  - 修复 `Label::setText()` 后尺寸与绘制范围不同步的问题。
  - 修复部分高频 hover 场景下日志过多、输入处理副作用偏大的问题。

- **修复 Dialog 生命周期中的历史遗留状态问题：**
  - 收口 `Dialog::Show()` 与 `Dialog::Close()` 的内部状态管理。
  - 清理冗余关闭状态与重复清理分支，使模态与非模态路径更加一致。

### ⚠️ 可能不兼容

- **Dialog 历史初始化接口已移除：**
  - `Dialog::setInitialization(bool)` 已移除。
  - 若旧代码仍直接调用该接口，请删除相关调用，并改为通过正常的：
    - `SetTitle(...)`
    - `SetMessage(...)`
    - `SetType(...)`
    - `Show()`
...
- **Window 尺寸 getter 的语义与旧版本不同：**
  - 若旧代码曾将 `Window::getWidth()` / `getHeight()` 当作“待收口尺寸”使用，升级后需要改用：
    - `getPendingWidth()`
    - `getPendingHeight()`
...
1. **Dialog 初始化相关调用迁移**
   - 删除对 `Dialog::setInitialization(...)` 的直接调用。
   - 内容变化统一通过：
     - `SetTitle(...)`
     - `SetMessage(...)`
     - `SetType(...)`
...
2. **Window 尺寸接口迁移**
   - 需要获取当前窗口实际客户区尺寸时，继续使用：
     - `getWidth()`
     - `getHeight()`
   - 需要获取 resize 收口前记录的待处理尺寸时，请改用：
     - `getPendingWidth()`
     - `getPendingHeight()`

3. **Dialog 覆盖区域内的交互预期调整**
   - 非模态对话框覆盖区域内的底层控件不再响应鼠标事件。
   - 若旧业务依赖对话框上的“点穿透”或 hover 穿透行为，需要按新语义调整。
...
- 锚点与布局系统的大改计划未纳入本次版本，将在后续版本单独推进。
